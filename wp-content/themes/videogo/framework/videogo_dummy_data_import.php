<?php
	...
	*/
	...
	add_action('wp_ajax_dummydata_import','videogo_dummydata_import');
	function videogo_dummydata_import(){
			...
			<div class="sidebar-wraper span2">
				<div class="sidebar-sublinks">
						<ul id="wp_t_o_right_menu">
							<li class="news_letter" id="active_tab"><?php esc_html_e('Dummy Content Settings', 'videogo'); ?></li>
						</ul>
				</div>
			</div>
							...
							<ul class="download_newsletter recipe_class row-fluid">
								<li class="panel-input span8">
									<span class="panel-title">
										<h3><?php esc_html_e('Import Dummy Content', 'videogo'); ?> </h3>
									</span>
									<input type="hidden" value="<?php echo esc_attr(wp_create_nonce ('themeple_nonce_import_dummy_data'));?>" name="themeple-nonce-dummy">
									<a class="themeple_btn themeple_btn_active themeple_dummy_data"><input type="button" value="<?php esc_html_e('Import Content', 'videogo'); ?>" id="importcontent" /></a>
								...
								<li class="span4 right-box-sec"><p><?php esc_html_e('Click on Import button to import your theme dummy content, After Clicking here everything will be set as demo. *Note: If dummy content display any error please make sure your Allowed memory size greater than 64MB.','videogo');?></p></li>
							</ul>
						</li>
					</ul>	
				</div>	
